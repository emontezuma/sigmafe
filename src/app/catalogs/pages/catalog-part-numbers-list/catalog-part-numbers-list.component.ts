...
@Component({
  ...
  templateUrl: './catalog-part-numbers-list.component.html',
  animations: [ routingAnimation, dissolve, ],
  styleUrls: ['./catalog-part-numbers-list.component.scss']
})
export class CatalogPartNumbersListComponent implements AfterViewInit {
...
// PartNumbers ===============
  ...
  filterByText: string;
  allPartNumbersToCsv: any;
  partNumbersData: PartNumbers = {
  ...
  ngOnInit() {
    ...
    this.searchBox$ = this._sharedService.search.pipe(
      tap((searchBox: SearchBox) => {
        if (searchBox.from === ApplicationModules.PART_NUMBERS_CATALOG) {
    ...
    this.toolbarClick$ = this._sharedService.toolbarAction.pipe(
      tap((buttonClicked: ToolbarButtonClicked) => {
        if (buttonClicked.from !== ApplicationModules.PART_NUMBERS_CATALOG) {
            return
        }
  ...
  ngAfterViewInit() {
    ...
    this.partNumbersCatalogData.paginator = this.paginator;
    this.partNumbersCatalogData.sort = this.sort;
...
// Functions ================
  ...
  requestData(skipRecords: number, takeRecords: number) {
    ...
    this.partNumbersData = {
      items: new Array(5).fill(emptyPartNumberCatalog),
    }
    let filter = null;
    ...
    this.partNumbersCatalogData = new MatTableDataSource<PartNumberItem>(this.partNumbersData.items);
    this.partNumbersData$ = this._catalogsService.getPartNumbersDataGql$(skipRecords, takeRecords, this.order, filter)
    .pipe(
      map((partNumbers: any) => {
        const { data } = partNumbers;
        return { 
          ...data,
          partNumbersPaginated: {
            ...data.partNumbersPaginated,
            items: data.partNumbersPaginated.items.map((item) => {
      ...
      tap( partNumbersData => {        
        this.setPaginator(partNumbersData.partNumbersPaginated.totalCount);
        this.partNumbersData = JSON.parse(JSON.stringify(partNumbersData.partNumbersPaginated));
        if (this.paginator) {
          ...
          if (this.pageInfo.currentPage * this.pageInfo.pageSize > 0) {
            this.partNumbersData.items = new Array(this.pageInfo.currentPage * this.pageInfo.pageSize).fill(null).concat(this.partNumbersData.items);
          }      
        }        
        this.partNumbersData.items.length = this.partNumbersData.totalCount;
        this.partNumbersCatalogData = new MatTableDataSource<PartNumberItem>(this.partNumbersData.items);
        this.partNumbersCatalogData.paginator = this.paginator;
        // this.partNumbersCatalogData.sort = this.sort;
        this.partNumbersCatalogData.sortData = () => this.partNumbersData.items;        
  ...
  toolbarAction(action: ToolbarButtonClicked) {
    if (action.from === ApplicationModules.PART_NUMBERS_CATALOG  && this.elements.length > 0) {
      if (action.action === ButtonActions.RELOAD) {
        ...
        this.elements.find(e => e.action === action.action).loading = true;                
        this._router.navigateByUrl("/catalogs/part-numbers/create");
        setTimeout(() => {
