...
import { CustomerItem, Customers, CustomersData, emptyCustomerCatalog } from '../../models';

@Component({
  selector: 'app-catalog-customers',
  templateUrl: './catalog-customers-list.component.html',
  animations: [ routingAnimation, dissolve, ],
  styleUrls: ['./catalog-customers-list.component.scss']
})
export class CatalogCustomersListComponent implements AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.searchBox$ = this._sharedService.search.pipe(
      tap((searchBox: SearchBox) => {
        if (searchBox.from === ApplicationModules.CUSTOMERS_CATALOG) {
    ...
    this.toolbarClick$ = this._sharedService.toolbarAction.pipe(
      tap((buttonClicked: ToolbarButtonClicked) => {
        if (buttonClicked.from !== ApplicationModules.CUSTOMERS_CATALOG) {
            return
        }
...
// Functions ================
  ...
  requestData(skipRecords: number, takeRecords: number) {
    ...
    this.customersData = {
      items: new Array(5).fill(emptyCustomerCatalog),
    }
    let filter = null;
    ...
    this.customersData$ = this._catalogsService.getCustomersDataGql$(skipRecords, takeRecords, this.order, filter)
    .pipe(
      map((customers: any) => {
      ...
      tap( customersData => {        
        this.setPaginator(customersData.customersPaginated.totalCount);
        this.customersData = JSON.parse(JSON.stringify(customersData.customersPaginated));
        if (this.paginator) {
  ...
  toolbarAction(action: ToolbarButtonClicked) {
    if (action.from === ApplicationModules.CUSTOMERS_CATALOG  && this.elements.length > 0) {
      if (action.action === ButtonActions.RELOAD) {
        ...
        this.elements.find(e => e.action === action.action).loading = true;                
        this._router.navigateByUrl("/catalogs/customers/create");
        setTimeout(() => {
        ...
        this.allCustomersToCsv$ = this._catalogsService.getAllToCsv$().pipe(
          tap(customersToCsv => {
            const fileData$ = this._catalogsService.getAllCsvData$(customersToCsv?.data?.exportCustomersToCsv?.exportedFilename)
            .subscribe(data => { 
              this.downloadFile(data, customersToCsv?.data?.exportCustomersToCsv?.downloadFilename);
              setTimeout(() => {
                this.elements.find(e => e.action === action.action).loading = false;
              }, 200);
            })
  ...
  //need to be extracted on refactor
  calcElements() {
    this.elements = [{
      type: 'button',
