...
import { PlantItem, Companies, CompaniesData, emptyPlantCatalog } from '../../models';

@Component({
  selector: 'app-catalog-companies',
  templateUrl: './catalog-companies-list.component.html',
  animations: [ routingAnimation, dissolve, ],
  styleUrls: ['./catalog-companies-list.component.scss']
})
export class CatalogCompaniesListComponent implements AfterViewInit {
...
// Functions ================
  ...
  requestData(skipRecords: number, takeRecords: number) {
    ...
    this.companiesData$ = this._catalogsService.getCompaniesDataGql$(skipRecords, takeRecords, this.order, filter)
    .pipe(
      map((companies: any) => {
        ...
        return { 
          ...
          companiesPaginated: {
            ...
            items: data.companiesPaginated.items.map((item) => {
      ...
      tap( companiesData => {        
        this.setPaginator(companiesData.companiesPaginated.totalCount);
        this.companiesData = JSON.parse(JSON.stringify(companiesData.companiesPaginated));
        if (this.paginator) {
        ...
        this.companiesCatalogData.paginator = this.paginator;
        // this.companiesCatalogData.sort = this.sort;
        this.companiesCatalogData.sortData = () => this.companiesData.items;        
  ...
  toolbarAction(action: ToolbarButtonClicked) {
    if (action.from === ApplicationModules.COMPANIES_CATALOG  && this.elements.length > 0) {
      if (action.action === ButtonActions.RELOAD) {
        ...
        this.elements.find(e => e.action === action.action).loading = true;                
        this._router.navigateByUrl("/catalogs/companies/create");
        setTimeout(() => {
