...
import { PlantItem, Lines, LinesData, emptyPlantCatalog } from '../../models';

@Component({
  selector: 'app-catalog-lines',
  templateUrl: './catalog-lines-list.component.html',
  animations: [ routingAnimation, dissolve, ],
  styleUrls: ['./catalog-lines-list.component.scss']
})
export class CatalogLinesListComponent implements AfterViewInit {
@ViewChild(MatPaginator) paginator: MatPaginator;
@ViewChild(MatSort) sort: MatSort;

// Lines ===============
  linesTableColumns: string[] = ['id', 'mainImagePath', 'name', 'reference', 'status', 'updatedAt'];
  linesCatalogData = new MatTableDataSource<PlantItem>([]);      
  
  linesData$: Observable<LinesData>;
  sort$: Observable<any>;
  ...
  ngOnInit() {
    ...
    this.searchBox$ = this._sharedService.search.pipe(
      tap((searchBox: SearchBox) => {
        if (searchBox.from === ApplicationModules.LINES_CATALOG) {
    ...
    this.toolbarClick$ = this._sharedService.toolbarAction.pipe(
      tap((buttonClicked: ToolbarButtonClicked) => {
        if (buttonClicked.from !== ApplicationModules.LINES_CATALOG) {
            return
        }
  ...
  ngAfterViewInit() {
    ...
    this.linesCatalogData.paginator = this.paginator;
    this.linesCatalogData.sort = this.sort;
...
// Functions ================
  ...
  requestData(skipRecords: number, takeRecords: number) {
    ...
    this.linesCatalogData = new MatTableDataSource<PlantItem>(this.linesData.items);
    this.linesData$ = this._catalogsService.getLinesDataGql$(skipRecords, takeRecords, this.order, filter)
    .pipe(
      map((lines: any) => {
        const { data } = lines;
        return { 
          ...
          linesPaginated: {
            ...data.linesPaginated,
            items: data.linesPaginated.items.map((item) => {
      ...
      tap( linesData => {        
        this.setPaginator(linesData.linesPaginated.totalCount);
        this.linesData = JSON.parse(JSON.stringify(linesData.linesPaginated));
        if (this.paginator) {
          ...
          if (this.pageInfo.currentPage * this.pageInfo.pageSize > 0) {
            this.linesData.items = new Array(this.pageInfo.currentPage * this.pageInfo.pageSize).fill(null).concat(this.linesData.items);
          }      
        }        
        this.linesData.items.length = this.linesData.totalCount;
        this.linesCatalogData = new MatTableDataSource<PlantItem>(this.linesData.items);
        this.linesCatalogData.paginator = this.paginator;
        // this.linesCatalogData.sort = this.sort;
        this.linesCatalogData.sortData = () => this.linesData.items;        
  ...
  toolbarAction(action: ToolbarButtonClicked) {
    if (action.from === ApplicationModules.LINES_CATALOG  && this.elements.length > 0) {
      if (action.action === ButtonActions.RELOAD) {
        ...
        this.elements.find(e => e.action === action.action).loading = true;                
        this._router.navigateByUrl("/catalogs/lines/create");
        setTimeout(() => {
  ...
  mapColumns() {    
    this.linesTableColumns = ['id', 'mainImagePath', 'name', 'reference', 'status', 'updatedAt'];
  }
