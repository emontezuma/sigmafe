...
import { PartNumberDetail, PartNumberItem, emptyPartNumberItem } from '../../models';
...
@Component({
  selector: 'app-catalog-part-number-edition',
  templateUrl: './catalog-part-number-edition.component.html',
  animations: [ routingAnimation, dissolve, ],
  styleUrls: ['./catalog-part-number-edition.component.scss']
})
export class CatalogPartNumberEditionComponent {
  @ViewChild('catalogEdition') private catalogEdition: ElementRef;
  @ViewChild(MatPaginator) paginator: MatPaginator;  
  @ViewChild('f') private thisForm: NgForm;

  // PartNumbers ===============
  partNumber: PartNumberDetail = emptyPartNumberItem;
  ...
  partNumberData: PartNumberItem;  
  goTopButtonTimer: any;
  takeRecords: number;
  focusThisField: string = '';

  partNumberForm = new FormGroup({
...
// Hooks ====================
  ngOnInit() {
    ...
    this.toolbarClick$ = this._sharedService.toolbarAction.pipe(
      ...
      tap((buttonClicked: ToolbarButtonClicked) => {      
        if (buttonClicked.from !== ApplicationModules.PART_NUMBERS_CATALOG_EDITION) {
            return
        }
    ...
    this.parameters$ = this._route.params.pipe(
      tap((params: Params) => {
        if (params['id']) {
          this.requestPartNumberData(+params['id']);
        }
      })
    ); 
    this.calcElements();
  ...
  ngOnDestroy() : void {
    ...
    if (this.partNumberFormChangesSubscription) this.partNumberFormChangesSubscription.unsubscribe(); 
  }
...
// Functions ================
  ...
  toolbarAction(action: ToolbarButtonClicked) {
    if (action.from === ApplicationModules.PART_NUMBERS_CATALOG_EDITION && this.elements.length > 0) {
      if (action.action === ButtonActions.NEW) {        
        ...
        setTimeout(() => {
          this.elements.find(e => e.action === action.action).loading = false;
          this._router.navigateByUrl('/catalogs/part-numbers'); 
        }, 750);
        ...
        if (!this.partNumber.id || this.partNumber.id === null || this.partNumber.id === 0) {
          ...
          noData = false;
          this.requestPartNumberData(this.partNumber.id);
        }
        ...
        this.elements.find(e => e.action === action.action).loading = true;
        if (this.partNumber?.id > 0 && this.partNumber.status === RecordStatus.ACTIVE) {
          const dialogResponse = this._dialog.open(GenericDialogComponent, {
            width: '450px',
            disableClose: true,
            panelClass: 'warn-dialog',
            ...
            data: {
              ...
              body: {
                message: $localize`Esta acción inactivará el Id <strong>${this.partNumber.id}</strong> y ya no estará activo en el sistema.<br><br><strong>¿Desea continuar?</strong>`,
              },
              showCloseButton: true,
            },
          });
          dialogResponse.afterClosed().subscribe((response) => {
            if (response.action === 'cancel') {
              ...
              this.elements.find(e => e.action === action.action).loading = true;
              const partNumberParameters = {
                settingType: 'status',
                id: this.partNumber.id,
                customerId: this.partNumber.customerId,
                status: RecordStatus.INACTIVE,
              }
              const partNumbers = this._sharedService.setGraphqlGen(partNumberParameters);
              this.updatePartNumber$ = this._catalogsService.updatePartNumberStatus$(partNumbers)
              .pipe(
                tap((data: any) => {
        ...
        } else if (this.partNumber?.id > 0 && this.partNumber.status === RecordStatus.INACTIVE) {
          const dialogResponse = this._dialog.open(GenericDialogComponent, {
            width: '450px',
            disableClose: true,
            autoFocus : true,
            data: {
              ...
              body: {
                message: $localize`Esta acción reactivará el Id <strong>${this.partNumber.id}</strong> y volverá a estar disponible en el sistema.<br><br><strong>¿Desea continuar?</strong>`,
              },
              showCloseButton: true,
            },
          });
        ...
        if (this.partNumber?.id > 0) {
          const dialogResponse = this._dialog.open(TranslationsDialogComponent, {
            ...
            data: {
              ...
              title: $localize`Traducciones <strong>${this.partNumber.id}</strong>`,
              topIcon: 'world',
              translations: this.partNumber.translations,
              buttons: [{
              ...
              body: {
                message: $localize`Esta acción inactivará al id ${this.partNumber.id} y ya no estará activo en el sistema.<br><br><strong>¿Desea continuar?</strong>`,
              },
  ...
  onSubmit() {
    ...
    if (this.partNumberForm.valid) {      
      this.saveRecord();   
    } else {
      let fieldsMissing = '';
      let fieldsMissingCounter = 0;
      for (const controlName in this.partNumberForm.controls) {
        if (this.partNumberForm.controls.hasOwnProperty(controlName)) {
          const typedControl: AbstractControl = this.partNumberForm.controls[controlName]; 
  ...
  requestPartNumberData(partNumberId: number): void { 
    ...
    partNumbers = { partNumberId };

    const skipRecords = 0;
    const filter = JSON.parse(`{ "partNumberId": { "eq": ${partNumberId} } }`);
    const order: any = JSON.parse(`{ "language": { "name": "${'ASC'}" } }`);
    // let getData: boolean = false;
    this.setViewLoading(true); 
    this.partNumber$ = this._catalogsService.getPartNumberDataGql$({ 
      partNumberId, 
      skipRecords, 
      ...
      map(([ partNumberGqlData, partNumberGqlTranslationsData ]) => {
        return this._catalogsService.mapOnePartNumber({
          partNumberGqlData,  
          partNumberGqlTranslationsData,
        })
      }),
  ...
  requestGenericsData$(currentPage: number, skipRecords: number, catalog: string, filterStr: string = null): Observable<any> {    
    ...
    const partNumbers = this._sharedService.setGraphqlGen(partNumberParameters);
    return this._catalogsService.getGenericsLazyLoadingDataGql$(partNumbers).pipe();
  }
  ...
  prepareRecordToSave(newRecord: boolean): any {
    ...
    return  {
        ...
        status: newRecord ? RecordStatus.ACTIVE : this.partNumber.status,
  ...
  initForm(): void {
    ...
    this.translationChanged = false;
    this.partNumber = emptyPartNumberItem;
  ...
  initUniqueField(): void {
    this.partNumber.id = null;
    this.partNumber.createdBy = null;
    this.partNumber.createdAt = null;
    this.partNumber.updatedBy = null;
    this.partNumber.updatedAt = null; 
    this.partNumber.status = RecordStatus.ACTIVE; 
    this.partNumber.translations.map((t) => {
      return {
        ...t,
        id: null,
      }
  ...
  processTranslations$(partNumberId: number): Observable<any> { 
    ...
    if (differences) {
      ...
      const translationsToAdd = this.partNumber.translations.map((t: any) => {
        return {
          id: null,
          partNumberId,
